...
async fn main() {
    ...
    let mut network = ClientNetwork::new(args.ip_addr, args.n_nodes as usize, bootnodes, rx_commands).await;
    tokio::spawn(async move {network.run().await});
    loop {
        tx_commands.unbounded_send("2 PRINT 0 1 0".to_string()).unwrap();
        tx_commands.unbounded_send("PRINT 0 1 1".to_string()).unwrap();
        tx_commands.unbounded_send("2 PRINT 0 1 2".to_string()).unwrap();
        tx_commands.unbounded_send("TRANSFER 0 2 1 3".to_string()).unwrap();
